...
func newWithName(name string, opts ...zap.Option) Logger {
	...
	if isLogRotation {
		logOuputPath := name + ".%Y-%m-%d.log"
		rotator, err := rotatelogs.New(
			logOuputPath,
			rotatelogs.WithMaxAge(30*24*time.Hour),    // delete after 30 days
		...
		logger = logger.WithOptions(
			zap.WrapCore(
				func(zapcore.Core) zapcore.Core {
					return zapcore.NewCore(zapcore.NewConsoleEncoder(loggerConfig.EncoderConfig), zapcore.AddSync(rotator), loggerConfig.Level)
				}))
	}
